...
<div class="py-3">
    ...
    <div class="bg-green-200 mt-4 rounded-lg p-3 mb-3">
      ...
      <form action="{{ route('toko.produk.update', $produk->kode_produk) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex justify-between gap-2 mb-3">
            <input type="hidden" name="type_produk" id="typeProduk" value="{{ $produk->type_produk === 'AUTO' ? 1 : 2 }}">
          <button type="button" disabled id="produkAuto" onclick="handleTypeProduk(1)"
            class="w-1/2 opacity-65 hover:cursor-not-allowed {{ $produk->type_produk === 'AUTO' ? 'bg-blue-500 text-slate-50' : '' }} hover:bg-blue-500 hover:text-slate-50 rounded-lg px-3 py-2 font-bold shadow-lg">
            Auto
          </button>
          <button type="button" disabled id="produkManual" onclick="handleTypeProduk(2)"
            class="w-1/2 border opacity-65 hover:cursor-not-allowed {{ $produk->type_produk === 'MANUAL' ? 'bg-blue-500 text-slate-50' : '' }} hover:bg-blue-500 hover:text-slate-50 rounded-lg px-3 py-2 font-bold shadow-lg">
            Manual
          </button>
        </div>
        ...
        @if(session()->has('produkTempEdit'))
            @php
                $sessionProduk = session()->get('produkTempEdit');
            @endphp
                ...
                <div class="rounded-lg px-2 py-3 bg-white shadow-lg">
                    <div class="py-2 flex items-center">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input {{ intval($produk->status_referal) > 0 ? 'checked' : '' }} onclick="terapkanAffiliasi(this)" @error('status_affiliasi') checked @enderror name="status_affiliasi" type="checkbox" class="sr-only peer">
                    ...
                    <div @if(intval($produk->status_referal) > 0) style="display: block" @else style="display: none" @endif @error('status_affiliasi') style="display: block" @enderror id="groupAffiliasi">
                        ...
                        <div class="relative overflow-hidden w-full">
                            ...
                            <select name="komisi_affiliasi" id="komisiAffiliasi" class="w-full @error('komisi_affiliasi') border-red-500 @enderror overflow-hidden border rounded-lg focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm">
                                ...
                                <option {{ $produk->komisi_referal == '1' ? 'selected' : '' }} value="1">1%</option>
                                <option {{ $produk->komisi_referal == '2' ? 'selected' : '' }} value="2">2%</option>
                                <option {{ $produk->komisi_referal == '3' ? 'selected' : '' }} value="3">3%</option>
                                <option {{ $produk->komisi_referal == '4' ? 'selected' : '' }} value="4">4%</option>
                                <option {{ $produk->komisi_referal == '5' ? 'selected' : '' }} value="5">5%</option>
                                <option {{ $produk->komisi_referal == '6' ? 'selected' : '' }} value="6">6%</option>
                                <option {{ $produk->komisi_referal == '7' ? 'selected' : '' }} value="7">7%</option>
                                <option {{ $produk->komisi_referal == '8' ? 'selected' : '' }} value="8">8%</option>
                                <option {{ $produk->komisi_referal == '9' ? 'selected' : '' }} value="9">9%</option>
                                <option {{ $produk->komisi_referal == '10' ? 'selected' : '' }} value="10">max: 10%</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="py-2 flex items-center">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input {{ isset($produk->garansi) ? 'checked' : '' }} onclick="terapkanGaransi(this)" name="status_garansi" @error('status_garansi') checked @enderror type="checkbox" class="sr-only peer">
                            <div class="w-8 h-4 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[3px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                        </label>
                    </div>
                    <div id="groupGaransi" @if(isset($produk->garansi)) style="display: block" @else style="display: none" @endif @error('status_affiliasi') style="display: block" @enderror>
                        ...
                        <div class="relative overflow-hidden w-full">
                            ...
                            <select name="garansi" id="garansi" class="w-full border rounded-lg @error('garansi') border-red-500 @enderror focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm">
                                ...
                                <option {{ $produk->garansi == '1' ? 'selected' : '' }} value="1">1 Hari</option>
                                <option {{ $produk->garansi == '2' ? 'selected' : '' }} value="2">2 Hari</option>
                                <option {{ $produk->garansi == '3' ? 'selected' : '' }} value="3">3 Hari</option>
                                <option {{ $produk->garansi == '4' ? 'selected' : '' }} value="4">4 Hari</option>
                                <option {{ $produk->garansi == '5' ? 'selected' : '' }} value="5">5 Hari</option>
                                <option {{ $produk->garansi == '6' ? 'selected' : '' }}value="6">6 Hari</option>
                                <option {{ $produk->garansi == '7' ? 'selected' : '' }} value="7">7 Hari</option>
                            </select>
                        </div>
                    </div>
                    @error('garansi')
                    ...
                    @if($produk->type_produk === 'MANUAL')
                        <div class="relative mt-3 overflow-hidden w-full">
                            ...
                            <select id="waktuProses" required name="waktu_proses" class="w-full @error('waktu_proses') border-red-500 @enderror border rounded-lg focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm">
                                ...
                                <option {{ $produk->waktu_proses == '1' ? 'selected' : '' }} value="1">1 Hari</option>
                                <option {{ $produk->waktu_proses == '2' ? 'selected' : '' }} value="2">2 Hari</option>
                                <option {{ $produk->waktu_proses == '3' ? 'selected' : '' }} value="3">3 Hari</option>
                                <option {{ $produk->waktu_proses == '4' ? 'selected' : '' }} value="4">4 Hari</option>
                                <option {{ $produk->waktu_proses == '5' ? 'selected' : '' }} value="5">5 Hari</option>
                                <option {{ $produk->waktu_proses == '6' ? 'selected' : '' }} value="6">6 Hari</option>
                                <option {{ $produk->waktu_proses == '7' ? 'selected' : '' }} value="7">7 Hari</option>
                            </select>
                        </div>
                        @error('waktu_proses')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            @else
                <div class="rounded-lg px-2 py-3 bg-white shadow-lg">
                    ...
                    <div class="relative overflow-hidden w-full">
                        ...
                        <input value="{{ $produk->nm_produk }}" required value="{{ $produk->nama_produk }}" name="nama_produk" type="text" class="w-full border rounded-lg @error('nama_produk') border-red-500 @enderror focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm" placeholder="Nama Produk">
                    </div>
                    @error('nama_produk')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                    <div class="relative mt-3 overflow-hidden w-full">
                        ...
                        <select required name="kategori" class="w-full border rounded-lg @error('kategori') border-red-500 @enderror focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm">
                            ...
                            @foreach(App\Models\Kategori::where('an', 1)->get() as $key => $value)
                                <option {{ $produk->kategori->kode_kategori == $value->kode_kategori ? 'selected' : '' }} value="{{ $value->kode_kategori }}">{{ $value->nama_kategori }}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('kategori')
                    ...
                    <div class="py-2 flex items-center">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer me-2">
                            <input {{ ($produk->potongan_harga > 0 || $produk->potongan_persen > 0) ? 'checked' : '' }} onclick="terapkanDiskon(this)" @error('potongan_persen') checked @enderror @error('potongan_harga') checked @enderror name="status_diskon" type="checkbox" class="sr-only peer">
                            <div class="w-8 h-4 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[3px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                        </label>
                    ...
                    <div @if($produk->potongan_harga > 0 || $produk->potongan_persen > 0) style="display: block" @else style="display: none" @endif @error('potongan_persen') style="display: block" @enderror @error('potongan_harga') style="display: block" @enderror id="groupDiskon">
                        <ul class="flex text-sm mb-3 rounded-ss-xl rounded-es-xl rounded-ee-xl rounded-se-xl overflow-hidden w-max">
                            <li id="diskonPersen" class="px-3 {{ ($produk->potongan_persen > 0) ? 'bg-blue-600 text-slate-50' : '' }} @error('potongan_persen') bg-blue-600 text-slate-50 @enderror py-2 border hover:text-slate-50 hover:bg-blue-600 hover:cursor-pointer">Persen %</li>
                            <li id="diskonHarga" class="px-3 {{ ($produk->potongan_harga > 0) ? 'bg-blue-600 text-slate-50' : '' }} py-2 @error('potongan_harga') bg-blue-600 text-slate-50 @enderror border hover:text-slate-50 hover:bg-blue-600 hover:cursor-pointer">Harga Rp.</li>
                        </ul>
                        <input type="hidden" id="typePotongan" name="type_potongan">
                        <div id="inputPotonganPersen" @if($produk->potongan_persen > 0) style="display: block" @else style="display: none" @endif @error('potongan_persen') style="display: block" @enderror class="mt-3 relative overflow-hidden w-full">
                            ...
                            <input value="{{ $produk->potongan_persen > 0 ? intval($produk->potongan_persen) : null }}" id="potonganPersen" name="potongan_persen" type="number" min="1" max="100" class="w-full border rounded-lg focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm" placeholder="cth : 50">
                        </div>
                        @error('potongan_persen')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                        <div id="inputPotonganHarga" @if($produk->potongan_harga > 0) style="display: block" @else style="display: none" @endif @error('potongan_harga') style="display: block" @enderror class="mt-3 relative overflow-hidden w-full">
                            <label class="text-[0.7em] absolute top-2 left-4 text-blue-600">Potongan Harga</label>
                            <input value="{{ $produk->potongan_harga > 0 ? intval($produk->potongan_harga) : null }}" id="potonganHarga" type="number" name="potongan_harga" min="3000" class="w-full @error('potongan_harga') border-red-500 @enderror border rounded-lg focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm" placeholder="cth : 20000">
                        </div>
                    ...
                    <div class="relative mt-3 overflow-hidden w-full">
                        ...
                        <input value="{{ intval($produk->harga) }}" name="harga" min="5000" placeholder="cth : 200000" type="number" class="w-full @error('harga') border-red-500 @enderror border rounded-lg focus:border-red-500 hover:bg-slate-200 h-full p-4 pt-6 focus:outline-none text-sm">
                    </div>
                    @error('harga')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                    ...
                    <div id="wrapperImage" class="relative overflow-hidden bg-slate-100 flex justify-start flex-wrap p-2 w-full min-h-[180px] max-h-max border rounded-lg">
                        <div class="flex items-center w-full lg:w-max flex-wrap" id="wrapperPreviewImages">
                            @if(@count($produk->images) > 0)
                                @foreach($produk->images as $key => $value)
                                    <div class="h-[140px] w-full border relative z-40 lg:h-[160px] p-2 lg:w-[200px]">
                                        <div style="background-image: url({{ $value->url }})" class="bg-slate-300 bg-cover bg-center w-full border z-50 h-full rounded-lg relative">
                                            <input type="hidden" name="images[]" value="{{ $value->uuid }}">
                                            <button onclick="deleteImage({{ $key }})" type="button" class="absolute z-50 imageList bg-red-500 text-slate-50 top-2 right-2 shadow-lg opacity-75 hover:opacity-100 rounded-full w-[30px] h-[30px]">
