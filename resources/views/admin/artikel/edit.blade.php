@extends('layouts.main', ['title' => 'Edit Artikel'])
<style>
  thead tr th {
    font-size: 0.9em;
    white-space: nowrap;
...
<div id="content" class="py-4">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-6">
        <h1 style="font-size: 1.5em">Edit Artikel</h1>
      </div>
    </div>
    <div class="card">
      <div class="card-body">
        <form action="{{ route('admin.artikel.update', $artikel->slug) }}" method="POST" class="w-100">
            @csrf
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="slug">Nama</label>
                    <input value="{{ old('nama_display') ? old('nama_display') : $artikel->display_name }}" type="text" class="form-control @error('nama_display') is-invalid @enderror form-control-sm" id="nama_display" name="nama_display">
                    @error('nama_display')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input value="{{ old('title') ? old('title') : $artikel->title }}" onkeyup="generateSlug(event)" type="text" class="form-control @error('title') is-invalid @enderror form-control-sm" id="title" name="title">
                    @error('title')
                        <div class="invalid-fedback">
                            <span class="text-danger">{{ $message }}</span> 
                        </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input readonly value="{{ $artikel->slug }}" type="text" class="form-control @error('slug') is-invalid @enderror form-control-sm" id="slug" name="slug">
                    @error('slug')
                        <div class="invalid-fedback">
                            <span class="text-danger">{{ $message }}</span> 
                        </div>
                        ...
                        <div class="col-md-8" id="wrapperKategori">
                            <select class="form-control @error('kategori') is-invalid @enderror form-control-sm" name="kategori" id="kategori">
                                ...
                                @foreach($kategori as $key => $value)
                                    <option {{ $artikel->kategori_id == $value->id ? 'selected' : '' }} value="{{ $value->id }}">{{ $value->nama }}</option>
                                @endforeach
                            </select>
                            @error('kategori')
                                <div class="invalid-fedback">
                ...
                <div class="col-md-12 mb-3">
                    <textarea class="@error('body') is-invalid @enderror" name="body" id="body">{{ $artikel->body }}</textarea>
                    @error('body')
                        <div class="invalid-fedback">
                            <span class="text-danger">{{ $message }}</span> 
                        </div>
